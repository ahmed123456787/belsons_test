...
export class NewsService {
  ...
  getCategories(): Observable<string[]> {
    return this.http.get<Category[]>(`${this.apiUrl}/categories/`).pipe(
      map((categories: Category[]) => categories.map((c) => c.name)),
      catchError(() => of([]))
    );
  }
  ...
  getCountries(): Observable<string[]> {
    return this.http.get<Country[]>(`${this.apiUrl}/countries/`).pipe(
      map((countries: Country[]) => countries.map((c) => c.name)),
      catchError(() => of([]))
    );
  }

  getSources(): Observable<string[]> {
    return this.http.get<Source[]>(`${this.apiUrl}/sources/`).pipe(
      map((response) => response.map((s) => s.name)),
      catchError(() => of([]))
    );
  }
}
